...
When running `wakey` for the first time, a SQLite database will be created to store your devices and groups. All data is managed through the application — use the built-in create, edit, and delete actions to manage your devices and groups.

The database is located in your home directory at `~/.wakey.db`.
...
- `MAC Address` — the MAC address of the device, used to send the Wake-on-LAN magic packet.
- `IP Address` — the IP address of the device, used to check whether it is online or offline.
- `State` — updated automatically by the application each time the devices view is loaded.
...
- `Devices` — the devices that belong to the group. Waking a group sends a magic packet to every device in it.
